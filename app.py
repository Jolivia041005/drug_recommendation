...
def get_dose_adjustment(eGFR_value):
...
def get_hepatic_use(hepatic_status):
    if not hepatic_status or hepatic_status == "无":
        return "可用"
    if "A" in hepatic_status:
        return "可用"
    if "B" in hepatic_status:
        return "减量慎用"
    if "C" in hepatic_status:
        return "禁用"
    return "可用"
...
with tab3:
    ...
    if run_recommend:
        ...
        st.info("请在左侧边栏输入患者特征后，点击「生成推荐方案」")

# 肝肾功能剂量调整
with tab4:
    st.subheader("肝肾功能剂量调整助手")

    st.markdown(
        """
        <div style="border:1px solid #FF9800; border-radius:6px; padding:12px; background-color:#FFF8E1; margin-bottom:16px;">
            <strong>功能说明：</strong>输入患者的肾功能（eGFR）和肝功能分级，系统将自动判断每种降糖药是否需要调整剂量或禁用。
    ...
    if st.button("评估所有药物的剂量调整建议", type="primary"):
        ...
        st.caption("按药物分类展示，颜色标识：绿色=可用，黄色=减量慎用，红色=禁用")

        results = []
        for _, row in df_drugs.iterrows():
            drug_name = row["drug_name_cn"]
            renal_advice = get_dose_adjustment(egfr)
            if child_pugh.startswith("A"):
                hepatic_advice = "可用"
            elif child_pugh.startswith("B"):
                hepatic_advice = "减量慎用"
            elif child_pugh.startswith("C"):
                hepatic_advice = "禁用"
            else:
                hepatic_advice = "可用"

            # 综合判断
            if renal_advice == "禁用" or hepatic_advice == "禁用":
                status = "禁用"
                color = "#FFEBEE"
                border = "#F44336"
            elif renal_advice == "减量慎用" or hepatic_advice == "减量慎用":
                status = "减量慎用"
                color = "#FFF8E1"
                border = "#FF9800"
            else:
        ...
        st.caption("提示：① eGFR 45-59需减量慎用，eGFR<15禁用；② Child-Pugh B级减量慎用，C级禁用；③ 具体药物需参考药品说明书。")
